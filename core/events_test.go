...
func TestMatchOfExpiredOccured(t *testing.T) {
	...
	newEvents, relatedEvents, _ := events.Match()
...
func TestOrphanedEvents(t *testing.T) {
	notification1 := &Pair[*int, *int]{First: IntRef(1), Second: IntRef(1)}
	notification2 := &Pair[*int, *int]{First: IntRef(2), Second: IntRef(1)}

	payment1 := &Pair[*int, *int]{First: IntRef(1), Second: IntRef(1)}
	payment2 := &Pair[*int, *int]{First: IntRef(3), Second: IntRef(1)}

	events := &Events[int, int, int]{
		Notifications:   []*Pair[*int, *int]{notification1, notification2},
		Payments:        []*Pair[*int, *int]{payment1, payment2},
		ExpireCondition: func(t *int) bool { return *t < 10 },
		NotificationWithPaymentMatchCondition: func(n, p *int) bool {
			return *n == *p
		},
	}

	_, _, orphanEvents := events.Match()

	assert.Equal(t, 1, len(orphanEvents.Notifications))
	assert.Equal(t, 2, *orphanEvents.Notifications[0])

	assert.Equal(t, 1, len(orphanEvents.Payments))
	assert.Equal(t, 3, *orphanEvents.Payments[0])
}
